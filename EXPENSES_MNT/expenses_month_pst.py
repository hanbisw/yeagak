...
class Expenses_month_pst_Window(QMainWindow):
    def __init__(self,pf_os,comp_code,user_id):
        super().__init__()
        self.move(10, 50)
        self.setFixedSize(1729, 903)
        if pf_os == 'Windows':
            option_ui = 'UiDir/expenses_month_pst.ui'
        else:
            option_ui = 'UiDir/expenses_month_pst.ui'
    ...
    def first_sch(self):
        ...
        mra = mariadb_conn().conn
        csr = mra.cursor()
        query_sel = "SELECT DATE_FORMAT(MIN(ilja),'%Y-%m-%d'), DATE_FORMAT(MAX(ilja),'%Y-%m-%d') from yeogak_reserve_day_tbl WHERE ilja LIKE '"+start_ilja_sch+"' "
        # print(query_sel)
        csr.execute(query_sel)
        rows = csr.fetchall()
        csr.close()
